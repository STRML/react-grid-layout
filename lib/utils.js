...
export function correctBounds(
  ...
  const noCollidesWith = getNoStatics(layout);

  // give static priority
  const orderedLayout = [...collidesWith, ...noCollidesWith]

  for (let i = 0, len = orderedLayout.length; i < len; i++) {
    const l = orderedLayout[i];
    // Overflows right
    ...
    // if (!l.static) collidesWith.push(l);
      ...
      while (getFirstCollision(orderedLayout, l)) {
        l.y++;
      }
    }
  }
...
/**
 * Get non-static elements.
 * @param  {Array} layout Array of layout objects.
 * @return {Array}        Array of non-static layout items..
 */
export function getNoStatics(layout) {
  return layout.filter(l => !l.static)
}
